...
INSERT INTO customer_transactions4
  (customer_id, txn_date, txn_type, txn_amount)
VALUES
...
UNION ALL
SELECT * FROM dbo.customer_nodes2
UNION ALL
SELECT * FROM dbo.customer_nodes3
UNION ALL    
SELECT * FROM dbo.customer_nodes4
UNION ALL
SELECT * FROM dbo.customer_nodes5
UNION ALL
SELECT * FROM dbo.customer_nodes6
)A
...
--Clean up duplicate in customer_transactions3 and customer_transaction4
DELETE FROM customer_transactions3 WHERE customer_id IN (SELECT DISTINCT customer_transactions3.customer_id
FROM customer_transactions3
JOIN customer_transactions4
ON customer_transactions3.customer_id = customer_transactions4.customer_id)

--a.2 What is the number of nodes per region?
SELECT regions.region_name, COUNT(node_id) AS [Number of nodes]
FROM #customer_nodes
JOIN regions
ON #customer_nodes.region_id = regions.region_id
GROUP BY regions.region_name
ORDER BY regions.region_name
--There are 713 nodes in Africa
--735 in America
--665 in Asia
--770 in Australia 
--616 in Europe
...
SELECT regions.region_name, COUNT(DISTINCT customer_id) AS [Number of customers]
FROM #customer_nodes
JOIN regions
ON #customer_nodes.region_id = regions.region_id
GROUP BY regions.region_name
...
SELECT AVG(DATEDIFF(DAY,start_date,end_date)) AS [Average days a customer rellocated]
FROM #customer_nodes
WHERE start_date != '9999-12-31' and end_date != '9999-12-31'
--Customers are rellocated to a different node averagely after 14 days

--a.5 What is the median, 80th and 95th percentile for this same reallocation days metric for each region?
DROP TABLE IF EXISTS #rellocation_days
SELECT region_id, DATEDIFF(DAY,start_date,end_date) AS [days a customer rellocated]
INTO #rellocation_days
FROM #customer_nodes
WHERE start_date != '9999-12-31' and end_date != '9999-12-31'
ORDER BY region_id

SELECT DISTINCT regions.region_name, PERCENTILE_CONT(0.5) WITHIN GROUP(ORDER BY [days a customer rellocated]) OVER(PARTITION BY #rellocation_days.region_id) AS [median], 
PERCENTILE_CONT(0.8) WITHIN GROUP(ORDER BY [days a customer rellocated]) OVER(PARTITION BY #rellocation_days.region_id) AS [80th percentile], 
PERCENTILE_CONT(0.95) WITHIN GROUP(ORDER BY [days a customer rellocated]) OVER(PARTITION BY #rellocation_days.region_id) AS [95th percentile]
FROM #rellocation_days
INNER JOIN regions
ON #rellocation_days.region_id = regions.region_id
--All the regions have the same median and 95th percentile, 15 and 28 days. However, there is a slight difference in the 80th percentile. Australia, Asia, and 
--America have the same result with 23 days, while Africe and Europe recorded 24 days.

--B. Customer Transactions
--b.1 What is the unique count and total amount for each transaction type?

--Create a union customer transactions table
DROP TABLE IF EXISTS #customer_transactions
SELECT *
INTO #customer_transactions
FROM
(
SELECT * FROM customer_transactions1
UNION ALL
SELECT * FROM customer_transactions2
UNION ALL 
SELECT * FROM customer_transactions3
UNION ALL
SELECT * FROM customer_transactions4
UNION ALL
SELECT * FROM customer_transactions5
UNION ALL
SELECT * FROM customer_transactions6
)B

SELECT DISTINCT txn_type, SUM(txn_amount) AS [Total amount], COUNT(txn_type) AS [Unique type count]
FROM #customer_transactions
GROUP BY txn_type
--Total amount for withdrawal is 793003 with 1580 unique count, for deposit is 1359168 with 2671 unique count, and for purchase is 806537 with 1617 unique count

--b.2 What is the average total historical deposit counts and amounts for all customers?
SELECT AVG(C.[historical deposit count]) AS [average historical deposit count], AVG(C.[total deposit amount]) AS [average total deposit amount]
FROM
(SELECT customer_id, COUNT(txn_date) AS [historical deposit count], SUM(txn_amount) AS [total deposit amount]
FROM #customer_transactions
WHERE txn_type = 'deposit'
GROUP BY customer_id
)C
--Average historical deposit count is 5 and average total deposit amount is 2718
...
SELECT [Months],[month name], COUNT(DISTINCT customer_id) AS [customer count]
FROM
(SELECT customer_id,DATEPART(MONTH,txn_date) AS [Months],DATENAME(MONTH,txn_date) AS [month name],SUM(CASE WHEN txn_type = 'deposit' THEN 1 ELSE 0 END) AS [Number of deposit], 
SUM(CASE WHEN txn_type = 'purchase' THEN 1 ELSE 0 END) AS [Number of purchase],
SUM(CASE WHEN txn_type = 'withdrawal' THEN 1 ELSE 0 END) AS [Number of withdrawal]
FROM #customer_transactions
GROUP BY customer_id, DATEPART(MONTH,txn_date), DATENAME(MONTH,txn_date)
)cus_month
WHERE [Number of deposit] > 1 AND ([Number of purchase] >=1 OR [Number of withdrawal] >=1)
GROUP BY [Months],[month name]
ORDER BY [Months]
--There is 168 customers in January, 181 in February, 192 in March, and 70 in April

--b.4 What is the closing balance for each customer at the end of the month?
SELECT customer_id,months, SUM([Closing balance]) AS [Closing balance]
FROM
(SELECT *, DATEPART(MONTH, txn_date) AS [months], CASE WHEN txn_type = 'deposit' THEN txn_amount ELSE -txn_amount END AS [Closing balance]
FROM #customer_transactions
)C
...
DROP TABLE IF EXISTS #all_customers_closing_balance
SELECT customer_id,months, SUM([Closing balance]) AS [Closing balance]
INTO #all_customers_closing_balance
FROM
(SELECT *, DATEPART(MONTH, txn_date) AS [months], CASE WHEN txn_type = 'deposit' THEN txn_amount ELSE -txn_amount END AS [Closing balance]
FROM #customer_transactions
)C
...
DROP TABLE IF EXISTS #monthly_percentage_change
SELECT *, (1.0 * ([Closing balance] - [Previous monthly closing balance])/[Previous monthly closing balance]) * 100 AS [percentage change]
INTO #monthly_percentage_change
FROM
(SELECT *, LAG([Closing balance]) OVER(PARTITION BY customer_id ORDER BY customer_id, months) AS [Previous monthly closing balance]
FROM #all_customers_closing_balance
)D
WHERE [Previous monthly closing balance] is NOT NULL
ORDER BY customer_id, months

SELECT 1.0 * 100 * COUNT(DISTINCT customer_id)/(SELECT COUNT(DISTINCT customer_id) FROM #customer_transactions) AS [>5% increase closing balance customers]
FROM #monthly_percentage_change 
WHERE [percentage change] > 5
--There is 53.8% of all the customers having their monthly balance increase more than 5% 

--C. Data Allocation Challenge
--c.1 To test out a few different hypotheses - the Data Bank team wants to run an experiment where 
...
DROP TABLE IF EXISTS #transaction_in_day
SELECT customer_id,txn_date, DATEPART(MONTH, txn_date) AS [Months], DATENAME(MONTH,txn_date) AS [Months Name],txn_type, txn_amount, 
SUM(CASE WHEN txn_type ='deposit' THEN txn_amount ELSE -txn_amount END) OVER(PARTITION BY customer_id ORDER BY txn_date) AS [Running balance]
INTO #transaction_in_day
FROM #customer_transactions
ORDER BY customer_id

SELECT *
FROM #transaction_in_day

DROP TABLE IF EXISTS #monthly_closing_balance
SELECT customer_id,txn_date,[Months],[Months Name], SUM(CASE WHEN txn_type ='deposit' THEN txn_amount ELSE -txn_amount END) AS [Monthly closing balance]
INTO #monthly_closing_balance
FROM #transaction_in_day
GROUP BY customer_id, [Months],[Months Name], txn_date

SELECT *
FROM #monthly_closing_balance

SELECT customer_id,MIN([Running balance]) AS [Minimum running balance], MAX([Running balance]) AS [Maximum running balance], AVG([Running balance]) AS [Average running balance]
INTO #min_max_avg_running_balance
FROM #transaction_in_day
GROUP BY customer_id
ORDER BY customer_id

SELECT *
FROM #min_max_avg_running_balance
ORDER BY customer_id

--Option 1: data is allocated based off the amount of money at the end of the previous month
SELECT B.[Months Name], SUM(B.[max running balance]) AS [Data Allocated]
FROM
(SELECT customer_id, [Months], [Months Name],MAX([Running balance]) AS [max running balance]
FROM #transaction_in_day
GROUP BY customer_id, [Months], [Months Name]
)B
GROUP BY B.[Months Name]
ORDER BY [Data Allocated] DESC
--January, February, and March require much less data than April, which means during these months, customers have higher
--closing balance than ones in April. The bank should optimize their business strategy by allocating more data for those three months 
-- and reducing the data allocated for April

--Option 2: data is allocated on the average amount of money kept in the account in the previous 30 days
SELECT Months, [Months Name], SUM([AVG running balance each month]) AS [Data Allocated]
FROM
(SELECT customer_id,Months,[Months Name], AVG([Running balance]) AS [AVG running balance each month]
FROM #transaction_in_day
GROUP BY customer_id, Months, [Months Name]
)op2
GROUP BY Months, [Months Name]
ORDER BY [Data Allocated] DESC
--When allocating data based on the average amount of money kept in the account in the last 30 days, customers tends to keep more moneny 
--in their accouns in January and February or they purchase or withdraw less than they deposit; meanwhile in March and April their balance
--tends to be negative, meaning they use more than they have

--Option 3: Data is updated real-time
SELECT Months, [Months Name], SUM([Running balance]) AS [Data Allocated]
FROM #transaction_in_day
GROUP BY Months, [Months Name]
ORDER BY [Data Allocated] DESC
--Customers in January and February tend to have positive running balance, meaning they have always use less than how much they deposited;
--while in April and March, they overuse the money they have, meaning there are more transactions happening during these months

--In conclusion, in January and February, customers always require less data than April and March. April always require more data since 
--in any options, customers in this month have negative balance. March has fluctuated data requirement based on the chose options. 
